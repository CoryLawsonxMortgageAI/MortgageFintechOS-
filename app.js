...
// ===================================================================
// ONTOLOGY ENGINE PAGE
// ===================================================================

const OE_PAGE_TITLES = {
    "ontology-engine": "Ontology Engine",
    "agent-builder": "Agent Builder",
    "predictive-intel": "Predictive Intelligence",
    "total-expert": "Total Expert CRM",
    "agent-diego": "DIEGO — Pipeline Orchestration",
    "agent-martin": "MARTIN — Document Intelligence",
    "agent-nova": "NOVA — Income & DTI Analysis",
    "agent-jarvis": "JARVIS — Condition Resolution",
    "agent-atlas": "ATLAS — Full-Stack Engineering",
    "agent-cipher": "CIPHER — Security Engineering",
    "agent-sentinel": "SENTINEL — System Intelligence",
};
Object.assign(PAGE_TITLES, OE_PAGE_TITLES);

async function refreshOntologyEngine() {
    try {
        const [stats, types, links, actions, graphData] = await Promise.all([
            fetchJSON("/api/ontology-engine/stats"),
            fetchJSON("/api/ontology-engine/types"),
            fetchJSON("/api/ontology-engine/links"),
            fetchJSON("/api/ontology-engine/actions"),
            fetchJSON("/api/ontology-engine/graph-export"),
        ]);

        // Stats
        const setEl = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
        setEl("oe-types-count", stats.object_types || 7);
        setEl("oe-links-count", stats.link_types || 9);
        setEl("oe-actions-count", stats.actions || 13);
        setEl("oe-objects-count", stats.total_objects || 0);

        // Object Types
        const typesList = document.getElementById("oe-types-list");
        if (typesList) {
            const entries = Object.entries(types);
            typesList.innerHTML = entries.map(([name, t]) => `
                <div style="padding:12px 20px;border-bottom:1px solid var(--border-light);display:flex;align-items:center;gap:16px;">
                    <div style="width:36px;height:36px;border-radius:8px;background:${t.color || '#58a6ff'}22;border:1px solid ${t.color || '#58a6ff'}44;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;color:${t.color || '#58a6ff'}">${name[0]}</div>
                    <div style="flex:1">
                        <div style="font-weight:700;font-size:14px;">${esc(name)}</div>
                        <div style="font-size:11px;color:var(--text-muted)">${t.property_count || 0} properties &bull; ${t.object_count || 0} objects</div>
                    </div>
                </div>
            `).join("");
        }
        ...
        // Links
        const linksList = document.getElementById("oe-links-list");
        if (linksList) {
            const lEntries = Object.entries(links);
            linksList.innerHTML = lEntries.map(([name, l]) => `
                <div class="flow-edge">
                    <span class="flow-node">${esc(l.from)}</span>
                    <span class="flow-arrow">&rarr;</span>
                    <span class="flow-label">${esc(l.label)}</span>
                    <span class="flow-arrow">&rarr;</span>
                    <span class="flow-node">${esc(l.to)}</span>
                    <span style="margin-left:auto;font-size:10px;color:var(--text-muted)">${esc(l.cardinality)}</span>
                </div>
            `).join("");
        }

        // Actions
        const actionsList = document.getElementById("oe-actions-list");
        if (actionsList) {
            const aEntries = Object.entries(actions);
            actionsList.innerHTML = aEntries.map(([name, a]) => `
                <div style="padding:10px 20px;border-bottom:1px solid var(--border-light);">
                    <div style="display:flex;align-items:center;gap:10px;">
                        <span style="font-weight:700;font-family:var(--font-mono);font-size:12px;color:var(--accent-bright)">${esc(name)}</span>
                        <span style="font-size:10px;padding:2px 8px;border-radius:4px;background:var(--bg-secondary);border:1px solid var(--border);color:var(--text-muted)">${esc(a.object_type)}</span>
                        <span style="margin-left:auto;font-size:10px;color:var(--text-muted)">Agent: ${esc(a.agent_hint || '--')}</span>
                    </div>
                    <div style="font-size:12px;color:var(--text-secondary);margin-top:4px;">${esc(a.description)}</div>
                </div>
            `).join("");
        }

        // Graph visualization
        const graphCanvas = document.getElementById("oe-graph-canvas");
        if (graphCanvas && graphData.nodes) {
            ...
            edges.forEach(e => {
                ...
                if (from && to) {
                    svg += `<line x1="${from.x}" y1="${from.y}" x2="${to.x}" y2="${to.y}" stroke="#30363d" stroke-width="1.5"/>`;
                    const mx = (from.x + to.x) / 2, my = (from.y + to.y) / 2;
                    svg += `<text x="${mx}" y="${my - 5}" text-anchor="middle" fill="#484f58" font-size="8">${esc(e.label)}</text>`;
                }
            });
            nodes.forEach(n => {
                const p = nodePositions[n.id];
                svg += `<circle cx="${p.x}" cy="${p.y}" r="28" fill="${n.color}22" stroke="${n.color}" stroke-width="2"/>`;
                svg += `<text x="${p.x}" y="${p.y - 5}" text-anchor="middle" fill="${n.color}" font-size="11" font-weight="700">${esc(n.id)}</text>`;
                svg += `<text x="${p.x}" y="${p.y + 10}" text-anchor="middle" fill="#8b949e" font-size="8">${n.object_count || 0} objects</text>`;
            });
            svg += `</svg>`;
        ...
        console.error("Ontology Engine refresh failed:", err);
    }
}
...
async function refreshAgentBuilder() {
    try {
        const [status, templates, agents] = await Promise.all([
            fetchJSON("/api/agent-builder/status"),
            fetchJSON("/api/agent-builder/templates"),
            fetchJSON("/api/agent-builder/agents"),
        ]);

        const setEl = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
        setEl("ab-total", status.total_definitions || 0);
        setEl("ab-running", status.running_agents || 0);
        setEl("ab-templates", status.templates_available || 5);

        // Templates
        const tList = document.getElementById("ab-templates-list");
        if (tList && templates.templates) {
            tList.innerHTML = templates.templates.map(t => `
                <div style="padding:14px 20px;border-bottom:1px solid var(--border-light);display:flex;align-items:center;gap:16px;">
                    <div style="flex:1">
                        <div style="font-weight:700;font-size:13px;">${esc(t.name)}</div>
                        <div style="font-size:11px;color:var(--text-muted);margin-top:2px;">${esc(t.description)}</div>
                        <div style="font-size:10px;color:var(--text-muted);margin-top:4px;">Codename: <strong>${esc(t.codename)}</strong> &bull; ${t.skills_count} skills</div>
                    </div>
                    <button class="btn btn-sm btn-primary" onclick="abDeployTemplate('${esc(t.template_id)}')">Deploy</button>
                </div>
            `).join("");
        }

        // Agents
        const aList = document.getElementById("ab-agents-list");
        if (aList && agents.agents) {
            if (agents.agents.length === 0) {
                aList.innerHTML = '<div class="empty-state">No custom agents yet. Deploy from a template above.</div>';
            } else {
                aList.innerHTML = agents.agents.map(a => `
                    <div style="padding:14px 20px;border-bottom:1px solid var(--border-light);display:flex;align-items:center;gap:16px;">
                        <div style="flex:1">
                            <div style="font-weight:700;font-size:13px;">${esc(a.name)} <span style="font-family:var(--font-mono);color:var(--accent-bright)">${esc(a.codename)}</span></div>
                            <div style="font-size:11px;color:var(--text-muted);margin-top:2px;">${esc(a.description || '')}</div>
                            <div style="font-size:10px;color:var(--text-muted);margin-top:4px;">${a.skills_count} skills &bull; v${a.version}</div>
                        </div>
                        <span class="status-badge ${a.running ? 'healthy' : ''}" style="font-size:10px;">${a.running ? 'Running' : 'Stopped'}</span>
                        ${!a.running ? `<button class="btn btn-sm btn-primary" onclick="abDeployAgent('${esc(a.agent_id)}')">Deploy</button>` : `<button class="btn btn-sm btn-danger" onclick="abUndeployAgent('${esc(a.agent_id)}')">Stop</button>`}
                    </div>
                `).join("");
            }
        }
        ...
        console.error("Agent Builder refresh failed:", err);
    }
}

async function abDeployTemplate(templateId) {
    try {
        const result = await fetch("/api/agent-builder/from-template", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({template: templateId}),
        }).then(r => r.json());
        if (result.agent_id) {
            await fetch(`/api/agent-builder/agents/${result.agent_id}/deploy`, {method: "POST"});
        }
        refreshAgentBuilder();
    } catch (err) { console.error(err); }
}

async function abDeployAgent(agentId) {
    await fetch(`/api/agent-builder/agents/${agentId}/deploy`, {method: "POST"});
    refreshAgentBuilder();
}

async function abUndeployAgent(agentId) {
    await fetch(`/api/agent-builder/agents/${agentId}/undeploy`, {method: "POST"});
    refreshAgentBuilder();
}

async function abCreateFromTemplate() {
    const templates = await fetchJSON("/api/agent-builder/templates");
    if (templates.templates && templates.templates.length > 0) {
        const first = templates.templates[0];
        await abDeployTemplate(first.template_id);
    }
}

async function abCreateCustom() {
    const name = document.getElementById("ab-name")?.value;
    const codename = (document.getElementById("ab-codename")?.value || "").toUpperCase();
    const category = document.getElementById("ab-category")?.value || "custom";
    const desc = document.getElementById("ab-desc")?.value || "";
    const role = document.getElementById("ab-role")?.value || "";
    const goal = document.getElementById("ab-goal")?.value || "";
    if (!name || !codename) { alert("Name and Codename required"); return; }
    const def = {
        name, codename, category, description: desc,
        division: category === "mortgage" ? "Mortgage Ops" : "Custom",
        instructions: { role, goal, constraints: [], expertise: [], personality: "" },
        skills: [], triggers: [{ type: "manual", config: {} }],
        required_integrations: ["llm"],
        guardrails: { max_actions_per_hour: 50, require_approval_for: [], blocked_operations: ["delete_contact", "delete_loan"], data_access_scope: [] },
    };
    await fetch("/api/agent-builder/agents", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(def) });
    refreshAgentBuilder();
}
...
async function abValidate() {
    const name = document.getElementById("ab-name")?.value;
    const codename = (document.getElementById("ab-codename")?.value || "").toUpperCase();
    const def = { name, codename, category: document.getElementById("ab-category")?.value || "custom" };
    const result = await fetch("/api/agent-builder/validate", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(def) }).then(r => r.json());
    const el = document.getElementById("ab-validation-result");
    if (el) el.innerHTML = result.valid ? '<span style="color:var(--success)">Valid</span>' : `<span style="color:var(--danger)">Errors: ${result.errors.join(", ")}</span>`;
}
...
async function refreshPredictiveIntel() {
    try {
        const formula = await fetchJSON("/api/ontology-engine/formula");

        // Variables
        const varsEl = document.getElementById("pi-variables");
        if (varsEl && formula.formula) {
            const vars = formula.formula.variables;
            varsEl.innerHTML = Object.entries(vars).map(([k, v]) => `
                <div style="padding:8px 12px;background:var(--bg-secondary);border:1px solid var(--border);border-radius:6px;">
                    <span style="font-family:var(--font-mono);font-weight:700;color:var(--accent-bright)">${esc(k)}</span>
                    <span style="color:var(--text-secondary);font-size:12px;margin-left:8px;">${esc(v)}</span>
                </div>
            `).join("");
        }

        // Stage weights
        const baselines = document.getElementById("pi-baselines");
        if (baselines && formula.risk_baselines) {
            baselines.innerHTML = '<table><thead><tr><th>Stage</th><th>Avg Days</th><th>Fallout Rate</th><th>Doc Completeness</th></tr></thead><tbody>' +
                Object.entries(formula.risk_baselines).map(([stage, b]) => `
                    <tr>
                        <td><strong>${esc(stage)}</strong></td>
                        <td>${b.avg_days}d</td>
                        <td>${(b.fallout_rate * 100).toFixed(0)}%</td>
                        <td><div class="risk-bar"><div class="risk-fill ${b.doc_completeness > 0.8 ? 'low' : b.doc_completeness > 0.5 ? 'medium' : 'high'}" style="width:${b.doc_completeness * 100}%"></div></div> ${(b.doc_completeness * 100).toFixed(0)}%</td>
                    </tr>
                `).join("") + '</tbody></table>';
        }

        // Status transitions
        const transitions = document.getElementById("pi-transitions");
        if (transitions && formula.status_transitions) {
            transitions.innerHTML = '<table><thead><tr><th>From</th><th>Allowed Transitions</th></tr></thead><tbody>' +
                Object.entries(formula.status_transitions).map(([from, to]) => `
                    <tr>
                        <td><strong>${esc(from)}</strong></td>
                        <td>${to.length ? to.map(t => `<span class="agent-badge idle" style="margin:2px;">${esc(t)}</span>`).join("") : '<span style="color:var(--text-muted)">Terminal state</span>'}</td>
                    </tr>
                `).join("") + '</tbody></table>';
        }

        // Stage weights visual
        const pipeline = document.getElementById("pi-pipeline");
        if (pipeline && formula.stage_weights) {
            pipeline.innerHTML = '<div style="padding:20px;">' +
                Object.entries(formula.stage_weights).map(([stage, weight]) => `
                    <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
                        <span style="width:110px;font-size:12px;font-weight:600;">${esc(stage)}</span>
                        <div style="flex:1;height:20px;background:var(--border);border-radius:4px;overflow:hidden;">
                            <div style="height:100%;width:${weight * 100}%;background:${weight > 0.7 ? 'var(--success)' : weight > 0.3 ? 'var(--warning)' : 'var(--danger)'};border-radius:4px;transition:width 0.5s;"></div>
                        </div>
                        <span style="width:40px;text-align:right;font-family:var(--font-mono);font-size:12px;font-weight:700;">${(weight * 100).toFixed(0)}%</span>
                    </div>
                `).join("") + '</div>';
        }

        // Funnel
        const funnel = document.getElementById("pi-funnel");
        if (funnel) {
            try {
                const funnelData = await fetchJSON("/api/ontology-engine/analytics/funnel");
                if (funnelData.funnel && funnelData.funnel.length) {
                    const maxCount = Math.max(...funnelData.funnel.map(f => f.count), 1);
                    funnel.innerHTML = '<div style="padding:20px;">' + funnelData.funnel.map(f => `
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            <span style="width:90px;font-size:11px;">${esc(f.stage)}</span>
                            <div style="flex:1;height:16px;background:var(--border);border-radius:4px;overflow:hidden;">
                                <div style="height:100%;width:${(f.count / maxCount) * 100}%;background:var(--accent);border-radius:4px;"></div>
                            </div>
                            <span style="width:30px;font-family:var(--font-mono);font-size:11px;">${f.count}</span>
                        </div>
                    `).join("") + '</div>';
                } else {
                    funnel.innerHTML = '<div class="empty-state">No loan data ingested yet. Sync from Total Expert CRM.</div>';
                }
            } catch (e) {
                funnel.innerHTML = '<div class="empty-state">Funnel data unavailable</div>';
            }
        }

    } catch (err) {
        console.error("Predictive Intel refresh failed:", err);
    }
}

// ===================================================================
// TOTAL EXPERT CRM PAGE
// ===================================================================

async function refreshTotalExpert() {
    try {
        const status = await fetchJSON("/api/total-expert/status");
        const setEl = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
        setEl("te-status", status.connected || status.authenticated ? "Connected" : "Not Connected");
        setEl("te-rate", `${status.rate_limit || 100}/min`);
        setEl("te-blocked", status.blocked_attempts || 0);

        // Object counts from ontology
        const oeStats = await fetchJSON("/api/ontology-engine/stats");
        const countsEl = document.getElementById("te-object-counts");
        if (countsEl && oeStats.object_counts) {
            countsEl.innerHTML = Object.entries(oeStats.object_counts).map(([type, count]) => `
                <div class="metric-card" style="padding:14px;">
                    <div class="metric-label">${esc(type)}s</div>
                    <div class="metric-value" style="font-size:22px;">${count}</div>
                </div>
            `).join("");
        }
    } catch (err) {
        console.error("Total Expert refresh failed:", err);
    }
}

async function teSyncAll() {
    const logEl = document.getElementById("te-sync-log");
    if (logEl) logEl.innerHTML = '<div style="padding:20px;color:var(--text-muted);">Syncing...</div>';
    try {
        const result = await fetch("/api/total-expert/sync", {method: "POST"}).then(r => r.json());
        if (logEl) {
            logEl.innerHTML = `<div style="padding:16px 20px;">
                <div style="font-weight:700;margin-bottom:8px;">Sync completed at ${esc(result.timestamp || new Date().toISOString())}</div>
                ${Object.entries(result.sync_results || {}).map(([type, r]) => `<div style="font-size:12px;color:var(--text-secondary);margin-bottom:4px;">${esc(type)}: ${JSON.stringify(r)}</div>`).join("")}
            </div>`;
        }
        refreshTotalExpert();
    } catch (err) {
        if (logEl) logEl.innerHTML = `<div style="padding:20px;color:var(--danger);">Sync failed: ${err.message}</div>`;
    }
}
...
const AGENT_FULL_INFO = {
    DIEGO: { name: "DIEGO", role: "Pipeline Orchestration", division: "Mortgage Ops", description: "Manages loan triage (FHA/VA/CONV/USDA/JUMBO), stage progression, priority assignment, and bottleneck detection across the entire mortgage pipeline.", capabilities: ["Loan Triage", "Stage Progression", "Priority Assignment", "Bottleneck Detection", "Pipeline Analytics"], skills: ["Pipeline Check", "Loan Triage", "Stage Progression", "Bottleneck Detection"] },
    MARTIN: { name: "MARTIN", role: "Document Intelligence", division: "Mortgage Ops", description: "Handles document classification, OCR validation, fraud detection, GHOST OSINT borrower verification, and completeness auditing.", capabilities: ["Document Classification", "OCR Validation", "Fraud Detection", "Borrower Verification", "Completeness Audit"], skills: ["Doc Classification", "OCR Validation", "Fraud Scan", "Completeness Check"] },
    NOVA: { name: "NOVA", role: "Income & DTI Analysis", division: "Mortgage Ops", description: "W-2 dual-method per FHA HB 4000.1, Schedule C analysis, DTI with compensating factors, collections 5% rule.", capabilities: ["W-2 Dual-Method", "Schedule C Analysis", "DTI Calculation", "Compensating Factors", "Collections Analysis"], skills: ["Income Calculation", "DTI Analysis", "Collections Review"] },
    JARVIS: { name: "JARVIS", role: "Condition Resolution", division: "Mortgage Ops", description: "LOE drafting (employment gap, deposits, credit), condition mapping, compliance citations (FHA/FNMA/FHLMC), lifecycle tracking.", capabilities: ["LOE Drafting", "Condition Mapping", "Compliance Citations", "Lifecycle Tracking"], skills: ["LOE Draft", "Condition Tracking", "Citation Lookup"] },
    ATLAS: { name: "ATLAS", role: "Full-Stack Engineering", division: "Engineering", description: "LLM-powered API generation, feature building, database migrations, React/TypeScript scaffolding.", capabilities: ["API Generation", "Feature Building", "DB Migrations", "React Scaffolding", "TypeScript Interfaces"], skills: ["Component Scaffold", "API Builder"] },
    CIPHER: { name: "CIPHER", role: "Security Engineering", division: "Engineering", description: "OWASP scanning, SOC2/PCI/GLBA compliance, encryption auditing, CVE patching, PentAGI integration.", capabilities: ["OWASP Scanning", "SOC2 Compliance", "PCI Compliance", "Encryption Audit", "CVE Patching"], skills: ["OWASP Scan", "Compliance Check"] },
    SENTINEL: { name: "SENTINEL", role: "System Intelligence", division: "Intelligence", description: "Codebase scanning, trend analysis, reverse engineering, build plan generation, AutoResearch, security audits.", capabilities: ["Codebase Scanning", "Trend Analysis", "Reverse Engineering", "Build Planning", "AutoResearch"], skills: ["Anomaly Detection", "Codebase Scan"] },
};

function renderAgentDetail(agentName) {
    const info = AGENT_FULL_INFO[agentName];
    if (!info) return;
    const el = document.getElementById(`agent-detail-${agentName.toLowerCase()}`);
    if (!el) return;

    el.innerHTML = `
        <div class="card" style="margin-bottom:24px;">
            <div class="card-header">
                <div class="card-title">${esc(info.name)} — ${esc(info.role)}</div>
                <span class="status-badge healthy"><span class="status-dot"></span> Active</span>
            </div>
            <div class="card-body" style="padding:24px;">
                <div style="margin-bottom:16px;font-size:14px;color:var(--text-secondary);line-height:1.7;">${esc(info.description)}</div>
                <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">
                    <span class="agent-badge idle">${esc(info.division)}</span>
                </div>
            </div>
        </div>
        <div class="grid-2col">
            <div class="card">
                <div class="card-header"><div class="card-title">Capabilities</div></div>
                <div class="card-body">
                    ${info.capabilities.map(c => `<div style="padding:10px 20px;border-bottom:1px solid var(--border-light);font-size:13px;display:flex;align-items:center;gap:10px;">
            ...
            <div class="card">
                <div class="card-header"><div class="card-title">Skills</div></div>
                <div class="card-body">
                    ${info.skills.map(s => `<div style="padding:10px 20px;border-bottom:1px solid var(--border-light);font-size:13px;font-family:var(--font-mono);color:var(--accent-bright);">${esc(s)}</div>`).join("")}
                </div>
            </div>
        </div>
    `;
...
const _origRefreshPage = refreshCurrentPage;
refreshCurrentPage = function() {
    try {
        switch (currentPage) {
            case "ontology-engine": refreshOntologyEngine(); return;
            case "agent-builder": refreshAgentBuilder(); return;
            case "predictive-intel": refreshPredictiveIntel(); return;
            case "total-expert": refreshTotalExpert(); return;
        }
        // Agent detail pages
        if (currentPage.startsWith("agent-")) {
            const agentName = currentPage.replace("agent-", "").toUpperCase();
            renderAgentDetail(agentName);
            return;
        }
    } catch (e) {
        console.error("Extended page refresh failed:", currentPage, e);
    }
    _origRefreshPage();
};
